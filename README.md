...
noodle is an open-source, contextual search engine for developers - by developers.
...
noodle is a new paradigm in search. It leverages the power of similar contextual searches of other developers with matching context, while maintaining anonymity and protecting private and proprietary data. The solutions are scored in terms of context specificity.
...
### noodle search lives in the tools you use

The shell, vim, vscode, intellij, git, issue trackers. You don't have to leave these tools, jump to a browser to get search results. noodle also lives in the browser, but it isn't restricted.
...
noodle allows tagging of searches for answers with the best solution. This creates a permanent contextual search hyperlink, which can be commented on, versioned, and shared. It also becomes searchable and prioritized in the noodle meta-search. Information on the web is often inaccurate, but corrections can be made in the meta-web.
